...
/// BT.2020 primaries at D65, for the profile that would describe a wide-gamut
/// output encoding. See [`OutputColorSpace`] for why nothing emits one today.
const REC2020_PRIMARIES: [[f64; 2]; 3] = [[0.708, 0.292], [0.170, 0.797], [0.131, 0.046]];
const REC2020_DESCRIPTION: &str = "raw-autotune BT.2020 (sRGB transfer)";

/// The colour space of the samples this program hands to a consumer.
///
/// **This is the *output encoding*, which is not the same thing as
/// `--working-space`.** `WorkingSpace` selects the space the *intermediate*
/// stages compute in — analysis, chroma denoising, local white balance — and
/// [`crate::color::WorkingSpace::to_display`] converts back to sRGB primaries
/// before the tone curve runs, precisely so that the choice of working space
/// cannot change the rendering. So `--working-space rec2020` produces sRGB
/// output with fewer out-of-gamut intermediates, not Rec.2020 output.
///
/// [`Self::Rec2020`] therefore exists for the encoder contract rather than
/// because anything selects it today: a JPEG XL or HEVC caller has to signal
/// *something* explicit rather than assume, and the day an output-referred
/// wide-gamut path lands, the signal and its profile are already here.
/// [`Self::of_render`] is the single place that decides, and
/// `output_colour_space_is_srgb_for_every_working_space` pins today's answer.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize)]
#[serde(rename_all = "snake_case")]
pub enum OutputColorSpace {
    /// sRGB primaries, D65 white, sRGB transfer function. Everything this
    /// program currently emits, in every configuration.
    Srgb,
    /// BT.2020 primaries, D65 white, sRGB transfer function.
    Rec2020,
}
...
impl OutputColorSpace {
    ...
    /// What a render with these settings actually encodes.
    ///
    /// Constant today, and deliberately written as a function of the render
    /// settings anyway: a consumer that reads this field keeps working when the
    /// answer stops being constant, and a reader of this file can see at a
    /// glance which settings were considered.
    pub const fn of_render(
        _working_space: crate::color::WorkingSpace,
        _raw_color_path: crate::color::RawColorPath,
    ) -> Self {
        Self::Srgb
    }

    /// The matrix-shaper ICC profile describing this space.
    pub fn icc_profile(self) -> &'static [u8] {
        match self {
            Self::Srgb => srgb_icc_profile(),
            Self::Rec2020 => rec2020_icc_profile(),
        }
    }
}
...
pub fn srgb_icc_profile() -> &'static [u8] {
    ...
    PROFILE.get_or_init(|| build_icc_profile(SRGB_PRIMARIES, SRGB_WHITE, ICC_DESCRIPTION))
}

/// The same matrix-shaper profile over BT.2020 primaries.
///
/// Reachable only through [`OutputColorSpace::Rec2020`], which nothing selects
/// yet — see that type. The transfer curve is the sRGB one because that is what
/// `tone::render` encodes with regardless of primaries, so a wide-gamut output
/// would be BT.2020 primaries with an sRGB transfer, and this profile says so.
pub fn rec2020_icc_profile() -> &'static [u8] {
    static PROFILE: OnceLock<Vec<u8>> = OnceLock::new();
    PROFILE.get_or_init(|| build_icc_profile(REC2020_PRIMARIES, SRGB_WHITE, REC2020_DESCRIPTION))
}
...
fn build_icc_profile(primaries: [[f64; 2]; 3], white: [f64; 2], description: &str) -> Vec<u8> {
    ...
    // The colorants are the requested primaries taken to XYZ under their own
    // white, then Bradford-adapted to the profile connection space's D50. Derived
    // here rather than pasted in as a table so the chain from "primaries" to
    // "profile bytes" is visible; `icc_colorants_are_the_derived_srgb_matrix`
    // pins the resulting integers for sRGB.
    let adaptation = bradford_adaptation(xy_to_xyz(white), ICC_D50_XYZ);
    let colorants = multiply(&adaptation, &rgb_to_xyz(primaries, white));
...
mod tests {
    ...
    fn icc_profile_is_cached_and_stable() {
        ...
        assert_eq!(
            first,
            build_icc_profile(SRGB_PRIMARIES, SRGB_WHITE, ICC_DESCRIPTION).as_slice()
        );
    }

    /// The generalized builder must not have moved the sRGB profile.
    ///
    /// `icc_colorants_are_the_derived_srgb_matrix` already pins the colorant
    /// integers; this pins the whole byte string, which is what actually ships
    /// inside every JPEG, PNG and TIFF this program writes.
    #[test]
    fn the_rec2020_profile_is_a_distinct_valid_profile_and_srgb_is_unmoved() {
        let srgb = srgb_icc_profile();
        let wide = rec2020_icc_profile();
        assert_ne!(srgb, wide, "the two profiles must not be the same bytes");
        for profile in [srgb, wide] {
            assert_eq!(
                u32::from_be_bytes(profile[0..4].try_into().unwrap()) as usize,
                profile.len(),
                "the header's declared size must match the buffer"
            );
            assert_eq!(&profile[36..40], b"acsp", "ICC file signature");
        }
        assert_eq!(
            OutputColorSpace::Srgb.icc_profile().as_ptr(),
            srgb.as_ptr(),
            "OutputColorSpace must hand back the cached profile, not a copy"
        );
    }

    /// Today's answer, pinned. `--working-space` selects the *intermediate*
    /// space and is converted back before the tone curve, so nothing this
    /// program emits is wide-gamut. If this test ever has to change, every
    /// consumer reading `color_space` gets the new answer for free — which is
    /// the reason the field exists.
    #[test]
    fn output_colour_space_is_srgb_for_every_working_space() {
        use crate::color::{RawColorPath, WorkingSpace};
        for working in [WorkingSpace::Srgb, WorkingSpace::Rec2020] {
            for path in [RawColorPath::Owned, RawColorPath::Rawler] {
                assert_eq!(
                    OutputColorSpace::of_render(working, path),
